...
static char printable[] = {
  // podemos sacar las entradas de -1 y automaticamente se ponen en cero (y cambiar el checkeo)
...
void printKey(char key) {
  ...
  if (c > 0) { // si es un caracter imprimible (el cero no quiero que lo imprima tampoco por eso el > 0 y no != -1)
    ncPrintChar(c);
    putChar(c, 0xFFFFFF);
  } else { 
    switch (key) {
    ...
    case 0x0F: // tab
      ...
      break;
    case 0x1C: // enter
      ncNewline(); // un new line del driver y de la consola?
      newLine();
      break;
    default:
      break;
...
void keyboardHandler() {
  ...
  if (key == 0x2A || key == 0x36) { // L or R Shift pressed
   ...
   if (key == 0x3A){ // Caps Lock (el release de caps lock no lo manejo, solo me importa cuando se apreta)
